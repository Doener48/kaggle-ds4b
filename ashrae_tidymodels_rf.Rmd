---
title: "Data Science for Business - Week 09: Kaggle Competition on Prediction of Elecricity Consumption (Random Forest)"
...
```{r}
electricity_mod_01 <- rand_forest(mode = "regression", 
                                  mtry = tune(), 
                                  trees = tune()) %>%
  set_engine("ranger")
...
Next, we specify a `recipe` for data preprocessing and feature engineering. In general, trees require little data preprocessing (e.g, no normalization, no dummy coding).

```{r}
electricity_recipe <- 
  recipe(meter_reading ~ ., data = train) %>%
  step_sample(size = 0.2) %>% # sample from the training data for performance reasons
  step_mutate(
